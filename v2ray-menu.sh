...
function add-user() {
	...
	if grep -qw "$user" /etc/vps/v2ray/clients.txt; then
		echo -e ""
	...
	email=${user}@${domain}
    cat>/etc/vps/v2ray/tls.json<<EOF
      {
       "v": "2",
       "ps": "${user}@auf",
       "add": "${BUG}.${domain}",
...
EOF
    ...
    vmesslink1="vmess://$(base64 -w 0 /etc/vps/v2ray/tls.json)"
	echo -e "${user}\t${uuid}\t${exp}" >> /etc/vps/v2ray/clients.txt
    cat /etc/vps/v2ray/conf/02_VLESS_TCP_inbounds.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuid}'","add": "'${domain}'","flow": "xtls-rprx-direct","email": "'${email}'"}]' > /etc/vps/v2ray/conf/02_VLESS_TCP_inbounds_tmp.json
	mv -f /etc/vps/v2ray/conf/02_VLESS_TCP_inbounds_tmp.json /etc/vps/v2ray/conf/02_VLESS_TCP_inbounds.json
    cat /etc/vps/v2ray/conf/03_VLESS_WS_inbounds.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuid}'","email": "'${email}'"}]' > /etc/vps/v2ray/conf/03_VLESS_WS_inbounds_tmp.json
	mv -f /etc/vps/v2ray/conf/03_VLESS_WS_inbounds_tmp.json /etc/vps/v2ray/conf/03_VLESS_WS_inbounds.json
    cat /etc/vps/v2ray/conf/04_trojan_TCP_inbounds.json | jq '.inbounds[0].settings.clients += [{"password": "'${uuid}'","email": "'${email}'"}]' > /etc/vps/v2ray/conf/04_trojan_TCP_inbounds_tmp.json
	mv -f /etc/vps/v2ray/conf/04_trojan_TCP_inbounds_tmp.json /etc/vps/v2ray/conf/04_trojan_TCP_inbounds.json
    cat /etc/vps/v2ray/conf/05_VMess_WS_inbounds.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuid}'","alterId": 0,"add": "'${domain}'","email": "'${email}'"}]' > /etc/vps/v2ray/conf/05_VMess_WS_inbounds_tmp.json
...
vless://$uuid@$BUG.$domain:$tls?flow=xtls-rprx-splice&encryption=none&security=tls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@auf
vless://$uuid@$BUG.$domain:$tls?flow=xtls-rprx-direct&encryption=none&security=tls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@auf
vless://$uuid@$BUG.$domain:$tls?encryption=none&security=tls&sni=$BUG&type=ws&host=$BUG&path=/v2rayws#$user@auf
trojan://$uuid@$BUG.$domain:$tls?sni=$BUG#$user@auf
${vmesslink1}
EOF
...
EOF
    ...
    echo -e ""
	echo -e " Link VLESS SPLICE: vless://$uuid@$BUG.$domain:$tls?flow=xtls-rprx-splice&encryption=none&security=tls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@auf"
    echo -e ""
    echo -e " Link VLESS DIRECT: vless://$uuid@$BUG.$domain:$tls?flow=xtls-rprx-direct&encryption=none&security=tls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@auf"
    echo -e ""
	echo -e " Link VLESS WS: vless://$uuid@$BUG.$domain:$tls?encryption=none&security=tls&sni=$BUG&type=ws&host=$BUG&path=/v2rayws#$user@auf"
    echo -e ""
	echo -e " Link TROJAN: trojan://$uuid@$BUG.$domain:$tls?sni=$BUG#$user@auf"
    echo -e ""
...
function delete-user() {
	...
	uuid="$(cat /etc/vps/v2ray/clients.txt | grep -w "$user" | awk '{print $2}')"
...
function user-list() {
	...
	total=$(wc -l /etc/vps/v2ray/clients.txt | awk '{print $1}')
    echo -e "\e[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
...
function user-monitor() {
	...
	data=($(cat /etc/vps/v2ray/clients.txt | awk '{print $1}'));
	echo -e "\e[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
...
function show-config() {
	...
	uuid=$(cat /etc/vps/v2ray/clients.txt | grep -w "$user" | awk '{print $2}')
	domain=$(cat /etc/vps/xray/domain)
	exp=$(cat /etc/vps/v2ray/clients.txt | grep -w "$user" | awk '{print $3}')
	exp_date=$(date -d"${exp}" "+%d %b %Y")
...
function change-port() {
    ...
    if [[ -z $cek ]]; then
    sed -i "s/$tls/$tls1/g" /etc/vps/v2ray/conf/02_VLESS_TCP_inbounds.json
...
0) clear ; menu ;;	
x) exit ;;
*) echo -e "" ; echo "Wrong Number" ; sleep 1 ; v2ray-menu ;;
esac

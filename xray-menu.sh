...
function add-user() {
	...
	if grep -qw "$user" /etc/vps/xray/clients.txt; then
		echo -e ""
...
EOF
    ...
    vmesslink1="vmess://$(base64 -w 0 /etc/vps/xray/tls.json)"
	echo -e "${user}\t${uuid}\t${exp}" >> /etc/vps/xray/clients.txt
    cat /etc/vps/xray/conf/02_VLESS_TCP_inbounds.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuid}'","add": "'${domain}'","flow": "xtls-rprx-direct","email": "'${email}'"}]' > /etc/vps/xray/conf/02_VLESS_TCP_inbounds_tmp.json
	mv -f /etc/vps/xray/conf/02_VLESS_TCP_inbounds_tmp.json /etc/vps/xray/conf/02_VLESS_TCP_inbounds.json
    cat /etc/vps/xray/conf/03_VLESS_WS_inbounds.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuid}'","email": "'${email}'"}]' > /etc/vps/xray/conf/03_VLESS_WS_inbounds_tmp.json
	mv -f /etc/vps/xray/conf/03_VLESS_WS_inbounds_tmp.json /etc/vps/xray/conf/03_VLESS_WS_inbounds.json
    cat /etc/vps/xray/conf/04_trojan_TCP_inbounds.json | jq '.inbounds[0].settings.clients += [{"password": "'${uuid}'","email": "'${email}'"}]' > /etc/vps/xray/conf/04_trojan_TCP_inbounds_tmp.json
	mv -f /etc/vps/xray/conf/04_trojan_TCP_inbounds_tmp.json /etc/vps/xray/conf/04_trojan_TCP_inbounds.json
    cat /etc/vps/xray/conf/05_VMess_WS_inbounds.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuid}'","alterId": 0,"add": "'${domain}'","email": "'${email}'"}]' > /etc/vps/xray/conf/05_VMess_WS_inbounds_tmp.json
	mv -f /etc/vps/xray/conf/05_VMess_WS_inbounds_tmp.json /etc/vps/xray/conf/05_VMess_WS_inbounds.json
    cat <<EOF >>"/etc/vps/config-user/${user}"
vless://$uuid@$domain:$xtls?flow=xtls-rprx-direct&encryption=none&security=xtls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@auf
vless://$uuid@$domain:$xtls?flow=xtls-rprx-splice&encryption=none&security=xtls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@auf
vless://$uuid@$domain:$xtls?encryption=none&security=xtls&sni=$BUG&type=ws&host=$BUG&path=/xrayws#$user@auf
trojan://$uuid@$domain:$xtls?sni=$BUG#$user@auf
${vmesslink1}
EOF
    cat <<EOF >>"/etc/vps/config-url/${user}"
# =======================================
# XRAY CORE CONFIG MERLIN CLASH ASUS
# By auf telegram: @auf
# =======================================
proxies:
  - {name: VLess Splice ${user}@auf, server: $BUG.$domain, port: $xtls, type: vless, flow: xtls-rprx-splice, uuid: $uuid, cipher: auto, tls: true, skip-cert-verify: true, network: tcp, sni: $BUG, udp: true}
  - {name: VLess Direct ${user}@auf, server: $BUG.$domain, port: $xtls, type: vless, flow: xtls-rprx-direct, uuid: $uuid, cipher: auto, tls: true, skip-cert-verify: true, network: tcp, sni: $BUG, udp: true}
  - {name: VLess WS ${user}@auf, server: $BUG.$domain, port: $xtls, type: vless, flow: xtls-rprx-direct, uuid: $uuid, cipher: auto, tls: true, skip-cert-verify: true, network: ws, ws-path: /xrayws, ws-headers: {Host: $BUG}, sni: $BUG, udp: true}
  - {name: Trojan ${user}@auf, server: $domain, port: $xtls, type: trojan, password: $uuid, sni: $BUG, skip-cert-verify: true, udp: true}
  - {name: VMess ${user}@auf, server: $domain, port: $xtls, type: vmess, uuid: $uuid, alterId: 0, cipher: auto, tls: true, skip-cert-verify: true, network: ws, ws-path: /xrayvws, ws-headers: {Host: $BUG}, udp: true}
  - {name: VMess SNI ${user}@auf, server: $domain, port: $xtls, type: vmess, uuid: $uuid, alterId: 0, cipher: auto, tls: true, skip-cert-verify: true, network: ws, ws-path: /xrayvws, ws-headers: {Host: $BUG}, sni: $BUG, udp: true}
port: 3333
socks-port: 23456
redir-port: 23457
allow-lan: true
...
tproxy-port: 23458
EOF
	base64Result=$(base64 -w 0 /etc/vps/config-user/${user})
    echo ${base64Result} >"/etc/vps/config-url/${uuid}"
	...
	echo -e ""
	echo -e " Link VLESS SPLICE: vless://$uuid@$BUG.$domain:$xtls?flow=xtls-rprx-splice&encryption=none&security=xtls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@auf"
    echo -e ""
    echo -e " Link VLESS DIRECT: vless://$uuid@$BUG.$domain:$xtls?flow=xtls-rprx-direct&encryption=none&security=xtls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@auf"
    echo -e ""
	echo -e " Link VLESS WS: vless://$uuid@$BUG.$domain:$xtls?encryption=none&security=xtls&sni=$BUG&type=ws&host=$BUG&path=/xrayws#$user@auf"
    echo -e ""
	echo -e " Link TROJAN: trojan://$uuid@$BUG.$domain:$xtls?sni=$BUG#$user@auf"
    echo -e ""
...
function delete-user() {
	...
	uuid="$(cat /etc/vps/xray/clients.txt | grep -w "$user" | awk '{print $2}')"
...
function user-list() {
	...
	total=$(wc -l /etc/vps/xray/clients.txt | awk '{print $1}')
    echo -e "\e[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
...
function user-monitor() {
	...
	data=($(cat /etc/vps/xray/clients.txt | awk '{print $1}'));
	echo -e "\e[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
...
function show-config() {
    ...
    xtls="$(cat ~/log-install.txt | grep -w "XRAY VLESS XTLS SPLICE" | cut -d: -f2|sed 's/ //g')"
    link=$(cat /etc/vps/config-user/${user})
	uuid=$(cat /etc/vps/xray/clients.txt | grep -w "$user" | awk '{print $2}')
	domain=$(cat /etc/vps/xray/domain)
	exp=$(cat /etc/vps/xray/clients.txt | grep -w "$user" | awk '{print $3}')
	exp_date=$(date -d"${exp}" "+%d %b %Y")
...
function change-port() {
    ...
    if [[ -z $cek ]]; then
    sed -i "s/$xtls/$xtls1/g" /etc/vps/xray/conf/02_VLESS_TCP_inbounds.json
